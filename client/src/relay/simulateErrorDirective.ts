 ...
 */
export function parseSimulateErrorDirectives(query: string): SimulateErrorField[] {
  ...
  try {
    ...
    for (const definition of document.definitions) {
      if (definition.kind === Kind.OPERATION_DEFINITION) {
        processSelections(definition.selectionSet.selections, [], fragments, fields);
      }
    }
  } catch (error) {
  ...
  for (const selection of selections) {
    if (selection.kind === Kind.FIELD) {
      ...
      if (fragmentDef?.selectionSet) {
        processSelections(fragmentDef.selectionSet.selections, currentPath, fragments, results);
      }
    } else if (selection.kind === Kind.INLINE_FRAGMENT) {
      processSelections(selection.selectionSet.selections, currentPath, fragments, results);
    }
  }
}
...
function processField(
  ...
  const tagged = field.directives?.some((directive) => directive.name.value === DIRECTIVE_NAME);
  if (tagged) {
    results.push({ path: fieldPath });
  }

  if (field.selectionSet) {
    processSelections(field.selectionSet.selections, fieldPath, fragments, results);
  }
}
...
export function simulateFieldErrors(
  ...
  const modified = {
    ...response,
    data: { ...response.data },
    errors: "errors" in response && response.errors ? [...response.errors] : [],
  };

  for (const field of fields) {
    const nulledPaths = nullFieldAtPath(modified.data as Record<string, unknown>, field.path);
